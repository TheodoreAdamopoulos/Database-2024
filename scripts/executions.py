...
for episode_id in range(1, 1 + EPISODES):
    ...
    for i in range(1, 11):
      ...
      random_cuisine_id = random.randint(1, CUISINES)
      while random_cuisine_id in cuisine_set:
        random_cuisine_id = random.randint(1, CUISINES)
      cuisine_set.add(random_cuisine_id)
      available_recipes = [RECIPES_PER_CUISINE * (random_cuisine_id - 1) + k for k in range(1, RECIPES_PER_CUISINE+1)]
      random_recipe_id = random.choice(available_recipes)
      recipe_set.add(random_recipe_id)
      available_cooks = [t[0] for t in cook_cuisine if t[1] == random_cuisine_id]
      while all(c in cook_set for c in available_cooks) or random_recipe_id in [9, 10]:
        recipe_set.remove(random_recipe_id)
        random_recipe_id = random.choice(available_recipes)
        recipe_set.add(random_recipe_id)
        available_cooks = [t[0] for t in cook_cuisine if t[1] == random_recipe_id]
      random_cook_id = random.choice(available_cooks)
